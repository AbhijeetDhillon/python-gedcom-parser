...
def US42(i,f):
    ...
    for k , v in i.items():
        ...
        if(v['Birthday'] != "NA"):  #individual birthday dates
            ...
            ind_bday.append([k[1:-1],v['Birthday'],v['Birthday'].split('-')[0],v['Birthday'].split('-')[1],v['Birthday'].split('-')[2]])
            ...
            if(fib!=3):
                print("ERROR: INDIVIDUAL: US42: "+ k[1:-1]+" Birthday date is in illegitimate date format")
                # print([k[1:-1],v['Birthday'],v['Birthday'].split('-')[0],v['Birthday'].split('-')[1],v['Birthday'].split('-')[2]])

        fid=0
        if(v['Death'] != "NA"):   #individual death dates
            #id,birthday,year,month,date
            ind_dday.append([k[1:-1],v['Death'],v['Death'].split('-')[0],v['Death'].split('-')[1],v['Death'].split('-')[2]])
            ...
            if(fid!=3):
                print("ERROR: INDIVIDUAL: US42: "+ k[1:-1]+" Death date is in illegitimate date format")
    ...
    for k , v in f.items():
        ...
        if(v['Married'] != "NA"):  #family marriage date
            #id,birthday,year,month,date
            fam_mday.append([k[1:-1],v['Married'],v['Married'].split('-')[0],v['Married'].split('-')[1],v['Married'].split('-')[2]])
            ...
            if(ffm!=3):
                print("ERROR: FAMILY: US42: "+ k[1:-1]+" Marriage date is in illegitimate date format")
                # print([k[1:-1],v['Married'],v['Married'].split('-')[0],v['Married'].split('-')[1],v['Married'].split('-')[2]])

        ffd=0
        if(v['Divorced'] != "NA"):   #family divorce date
            #id,birthday,year,month,date
            fam_dday.append([k[1:-1],v['Divorced'],v['Divorced'].split('-')[0],v['Divorced'].split('-')[1],v['Divorced'].split('-')[2]])
            ...
            if(ffd!=3):
                print("ERROR: FAMILY: US42: "+ k[1:-1]+" Divorce date is in illegitimate date format")
            ...
            if(calcAge(v['Birthday']) < 18 and calcAge(v['Birthday']) > 0):
                ...
                for kf , vf in f.items():
                    ...
                    #   print(hid,wid)
                        ...
                        # print(hf,wf)
                        if(hf==1 and wf==1):
                            print("ERROR: INDIVIDUAL: US33: "+ k[1:-1]+" is an orphaned child")
